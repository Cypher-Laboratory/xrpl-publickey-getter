...
exports.getAddressFromSigningPubkey = exports.getSigningPubkeyFromLatestTx = exports.getLatestTx = exports.getPubKeysFromAddresses = void 0;
...
async function getPubKeysFromAddresses(addresses) {
    ...
    const SigningPubKeys = await Promise.all(addresses.map(async (address) => {
...
function getSigningPubkeyFromLatestTx(latestTx) {
    for (let i = 0; i < latestTx.length; i++) {
        ...
        if (getAddressFromSigningPubkey(signingPubKey) === latestTx[i].tx?.Account) {
            return signingPubKey;
        }
    }
...
function getAddressFromSigningPubkey(pubkeyHex) {
    ...
    const pubkey_inner_hash = (0, node_crypto_1.createHash)("sha256").update(pubkey);
    ...
    const chksum_hash1 = (0, node_crypto_1.createHash)("sha256").update(payload).digest();
    const chksum_hash2 = (0, node_crypto_1.createHash)("sha256").update(chksum_hash1).digest();
